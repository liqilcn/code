...
class T5TestDataset(Dataset):
    def __init__(self, args, config, tokenizer, mode='test', dataset_path='./delve'):
        ...
        for json_line in tqdm(jsonlines):
            ...
            # 这里和BART不同, 如果前面不添加一个<pad>, <sen-sep>将是第一个token，T5的tokenizer不会给<sen-sep>前面添加特殊token，而BART的会，这是经过jupyter验证的
            # 所以需要添加一个<pad>, 使得T5和BART的后续处理一致
...
if __name__ == "__main__":
    ...
    test_dataset = T5TestDataset(args, config=model_config, tokenizer=tokenizer, mode='train', dataset_path=args.dataset_path)
    ...
    with torch.no_grad():
        ...
        print('all_case_matrix变量结束使用,回收占用的内存')
        all_case_matrix = None
        ...
        def hyper_para_search_worker(all_arg):
            ...
            for i in range(len(all_scores_for_para_search)):
                ...
                for ii in range(len(top2_gd)):
                    ...
                    # if score_sum == 0:
                    #     score = [s/max(score) for s in score]
