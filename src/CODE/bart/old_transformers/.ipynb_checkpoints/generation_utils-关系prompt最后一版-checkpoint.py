...
class GenerationMixin:
    ...
    def generate(
        ...
        **model_kwargs
        ...
        if self.config.is_encoder_decoder:
            ...
            # 加了这里，beam搜索会自动将decoder的各项输入重复4次，同样将关系提示也重复4次（beam=4的情况）
            for i in range(len(encoder_outputs.relation_prompt_for_decoder)):
                for attention_type in encoder_outputs.relation_prompt_for_decoder[i]:
                    for kk in encoder_outputs.relation_prompt_for_decoder[i][attention_type]:
                        encoder_outputs.relation_prompt_for_decoder[i][attention_type][kk] = encoder_outputs.relation_prompt_for_decoder[i][attention_type][kk].index_select(0, expanded_batch_idxs)


            encoder_outputs["relation_prompt_for_decoder"] = encoder_outputs.relation_prompt_for_decoder
    ...
    def _generate_beam_search(
        ...
        # done sentences
        done = [False for _ in range(batch_size)]  # 用于标志当前batch的句子是否完成解码，即eos token是否产出

        all_decoder_input_ids_and_cross_attn = []
        while cur_len < max_length:
            model_inputs = self.prepare_inputs_for_generation(
                input_ids, past=past, attention_mask=attention_mask, use_cache=use_cache, **model_kwargs
            )
            
            outputs, all_cross_attns = self(**model_inputs, return_dict=True)  # (batch_size * num_beams, cur_len, vocab_size)
            all_decoder_input_ids_and_cross_attn.append({'decoder_input_ids':model_inputs['decoder_input_ids'], 'all_cross_attns':all_cross_attns})  # 加了这一行
            ...
            for batch_idx in range(batch_size):
                ...
                # if we are done with this sentence, add a pad token  # 如果句子生成已经结束，则在后面追加pad token
                if done[batch_idx]:
                    assert (
                ...
                for beam_token_rank, (beam_token_id, beam_token_score) in enumerate(
                    ...
                    effective_beam_id = batch_idx * num_beams + beam_id  # 同一个输入对应的不同解码结果
        ...
        output_num_return_sequences_per_batch = 1 if do_sample else num_return_sequences   # 可以自定义每个beam中返回的sequence数

        # select the best hypotheses
        sent_lengths = input_ids.new(output_batch_size)  # .new()新建一个type和device和目标tensor相同的tensor
        best = []

        # retrieve best hypotheses
        for i, hypotheses in enumerate(generated_hyps):
            sorted_hyps = sorted(hypotheses.beams, key=lambda x: x[0])  # 按照score排序，取出最好的seq；hypotheses.beams[0]是score
            for j in range(output_num_return_sequences_per_batch):  # 每个beam迭代最大保留个数次
        ...
        return decoded, all_decoder_input_ids_and_cross_attn
