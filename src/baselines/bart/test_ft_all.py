...
class BartForSequenceClassificationNew(BartPretrainedModel):
...
if __name__ == "__main__":
    ...
    flags.add_argument('-dataset_path',        default='../../CODE/bart/datasets_with_generated_summary', type=str)
    ...
    flags.add_argument('-test_dir',            default='', type=str)
    flags.add_argument('-decoder_layer',       default=5,    type=int)#与encoder信息融合的decoder层
    ...
    model_path = get_metrics(args.test_dir)
    print(model_path)
    # model_path = '/home/share/qli/jupyter/train_eval_super_Bart/finetune_all/ckpts/ft_all/Bart_large/delve_small_0.0001_10_8_/checkpoint-1750'

    model_config = BartConfig.from_pretrained(model_path, local_files_only=True)
    ...
    # 接下来就是进行测试，得到结果了
    test_dataset = BartDataset(args, config=model_config, tokenizer=tokenizer, mode='test', dataset_path=args.dataset_path)
    ...
    test_model = BartForSequenceClassificationNew.from_pretrained(model_path, local_files_only=True, ignore_mismatched_sizes=True, config=model_config)
