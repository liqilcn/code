...
if __name__ == "__main__":
    ...
    flags.add_argument('-batch_size',          default=4,    type=int)
    flags.add_argument('-gradient_accumulation_steps', default=1, type=int)
    flags.add_argument('-lr',                  default=3e-5,  type=float)  
    ...
    flags.add_argument('-seed',          default=1996, type=int)
    ...
    model_size = args.model_size.split('_')[1]
    # init_model_path = f'{args.init_model_path}/{model_size}/'
    init_model_path = f'{args.init_model_path}'
    ...
    if 'train' == args.mode:
        model = BartForSequenceClassificationNew.from_pretrained(init_model_path, local_files_only=True, ignore_mismatched_sizes=True, config=model_config)
        model.cuda()
    else:
