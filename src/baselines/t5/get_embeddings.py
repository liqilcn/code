...
class T5ExtractEembedding(T5PreTrainedModel):
...
if __name__ == "__main__":
    ...
    flags.add_argument('-lr',                  default=1e-5,  type=float)  # BART 3e-5 达到最佳
    flags.add_argument('-warmup_steps',        default=200,   type=int)
    flags.add_argument('-weight_decay',        default=1e-4,  type=float)
    flags.add_argument('-epochs',              default=50,    type=int)
    flags.add_argument('-num_workers',         default=2,    type=int)
    ...
    # %%
    if 'train' == args.mode:
        model = T5ExtractEembedding.from_pretrained(args.init_model_path, local_files_only=True, ignore_mismatched_sizes=True, config=model_config)
    ...
    # %%
    test_dataset = T5Dataset(args, config=model_config, tokenizer=tokenizer, mode='test',\
        dataset_path=args.dataset_path)
